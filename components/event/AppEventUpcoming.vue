<template>
  <v-container class="pa-0 ">
    ...
    <v-layout
      ...
      style="background-position: right background-image: url('~/svg/bg.svg')"
    >
      <v-flex v-for="n in 3" :key="`4${n}`" xs12 sm6 md4 lg4>
        <v-card
          v-if="showLoader"
          flat
    ...
    >
      <v-flex v-if="showLoader" xs12>
        <v-layout row wrap>
          <v-flex v-for="n in 2" :key="`4${n}`" xs12 sm6 md4 lg4>
