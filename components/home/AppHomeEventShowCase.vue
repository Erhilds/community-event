<template>
  <v-container class="pa-0">
    <v-snackbar v-model="errorAlert" bottom left>
      {{ errorMsg }}
      <v-btn color="pink" flat @click="errorAlert = false">
        Close
      </v-btn>
    </v-snackbar>
    ...
    >
      <v-flex v-if="showLoader" xs12>
        <v-layout row wrap>
          <v-flex v-for="n in 4" :key="`4${n}`" xs12 sm6 md3 lg3>
            <v-card
              flat
      ...
      <v-flex v-for="(item, i) in eventsData" :key="i" xs12 sm6 md3 lg3>
        <v-card
          flat
      ...
      <v-flex v-if="notFoundEventFlag == true" xs12 class="text-xs-center">
    ...
    >
      <v-flex v-if="showLoader" xs12>
        <v-layout row wrap>
          <v-flex v-for="n in 4" :key="`4${n}`" xs12 sm6 md4 lg4>
      ...
      <v-flex xs12>
        <v-slide-y-reverse-transition>
          <v-list v-show="showData" two-line subheader>
            <v-list-tile
